...
func (s *Store) GetChatSettings(ctx context.Context, chatID int64) (adminconfig.ChatSettings, bool, error) {
	...
	err := s.db.QueryRowContext(ctx, `
SELECT chat_id, alias, tone_mode, allow_teasing, interactivity_mode, updated_at, updated_by
FROM chat_settings
WHERE chat_id = ?
`, chatID).Scan(&settings.ChatID, &alias, &toneMode, &allow, &interactivity, &updatedAt, &settings.UpdatedBy)
	if err != nil {
...
INSERT INTO chat_settings(chat_id, alias, tone_mode, allow_teasing, interactivity_mode, updated_at, updated_by)
